...
- **Storage.** Segments are 1 minute (1200 ticks), Deflate-compressed, in `data/echoaholic/streams/<uuid>/<seq>.seg` with `index.bin`, written, read, decoded and deleted on the `Echoaholic-IO` thread. Echo entities are never saved to chunks. `/echoaholic clear` schedules a save of `world.dat` at once. An unreadable `world.dat` is copied to `world.dat.damaged-<ms>`, the mode stays OFF in memory and the data is not marked dirty, so the file is not overwritten with defaults (`EchoBootstrap`). Measured by `SegmentSizeTest`: about 389 KiB per dense hour, 2.2 KiB per idle hour, 2.3 MiB for the default 6 h buffer.
